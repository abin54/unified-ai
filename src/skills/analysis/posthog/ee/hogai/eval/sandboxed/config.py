...
class AgentArtifacts(BaseModel):
    ...
    git_diff: str = ""
    """Git diff extracted from src.orchestrator tool calls."""

    files_changed: list[str] = Field(default_factory=list)
    """File paths extracted from src.orchestrator tool calls."""

    test_exit_code: int | None = None
    """Inferred exit code from test tool calls (None if not run)."""

    test_output: str = ""
    """Test output extracted from src.orchestrator tool calls."""

    lint_exit_code: int | None = None
    """Inferred exit code from lint tool calls (None if not run)."""

    lint_output: str = ""
    """Lint output extracted from src.orchestrator tool calls."""
