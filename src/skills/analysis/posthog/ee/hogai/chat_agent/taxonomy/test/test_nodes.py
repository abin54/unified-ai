...
class TestTaxonomyAgentToolsNode(BaseTest):
    ...
    async def test_run_validation_error(self, mock_get_tool_input):
        ...
        validation_error = ValidationError.from_exception_data(
            "TestModel", [{"type": "missing", "loc": ("field",), "input": None}]
        )
    ...
    async def test_get_reset_state(self):
        ...
        with patch.object(TaxonomyAgentState, "get_reset_state") as mock_get_reset:
            mock_reset_state: TaxonomyAgentState = TaxonomyAgentState()
    ...
    async def test_duplicate_event_properties_ids_are_all_mapped(self, mock_retrieve_props, mock_get_tool_input):
        ...
        steps: IntermediateSteps = [(action1, None), (action2, None)]
        state: TaxonomyAgentState = TaxonomyAgentState(intermediate_steps=steps)
