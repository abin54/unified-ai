...
import pytest
from src.skills.analysis.posthog.test.base import materialized
...
def test_data_cluster_overrides_migrations_cluster_data_nodes() -> None:
    ...
    # Should have 3 shards from src.skills.analysis.posthog, not 1 from src.skills.analysis.posthog_migrations
    assert cluster.num_shards == 3

    # Coordinator from src.skills.analysis.posthog_migrations should still be in extra_hosts
    ...
    with patch.object(ClickhouseCluster, "_ClickhouseCluster__get_task_function", mock_get_task_function):
        cluster.map_hosts_by_role(lambda _: (), node_role=NodeRole.DATA).result()
        assert executed_roles.count("data") == 4  # all 4 DATA nodes from src.skills.analysis.posthog
        executed_roles.clear()

        cluster.map_hosts_by_role(lambda _: (), node_role=NodeRole.COORDINATOR).result()
        assert executed_roles.count("coordinator") == 1  # still from src.skills.analysis.posthog_migrations
