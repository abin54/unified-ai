...
if TYPE_CHECKING:
    from src.skills.analysis.posthog.event_usage import AnalyticsProps
...
from src.skills.analysis.posthog.clickhouse.client.limit import ConcurrencyLimitExceeded, limit_concurrency
from src.skills.analysis.posthog.clickhouse.query_tagging import Feature, Product, get_query_tags, tag_queries
from src.skills.analysis.posthog.cloud_utils import is_cloud
from src.skills.analysis.posthog.errors import CH_TRANSIENT_ERRORS, CHQueryErrorTooManySimultaneousQueries
from src.skills.analysis.posthog.exceptions_capture import capture_exception
from src.skills.analysis.posthog.metrics import pushed_metrics_registry
from src.skills.analysis.posthog.ph_client import get_regional_ph_client
from src.skills.analysis.posthog.redis import get_client
from src.skills.analysis.posthog.settings import CLICKHOUSE_CLUSTER
from src.skills.analysis.posthog.tasks.utils import CeleryQueue, PushGatewayTask
...
def delete_expired_exported_assets() -> None:
    from src.skills.analysis.posthog.models import ExportedAsset
...
def process_query_task(
    ...
    """
    from src.skills.analysis.posthog.clickhouse.client import execute_process_query
...
def clickhouse_clear_removed_data() -> None:
    from src.skills.analysis.posthog.models.async_deletion.delete_cohorts import AsyncCohortDeletion
...
def calculate_cohort(parallel_count: int) -> None:
    from src.skills.analysis.posthog.tasks.calculate_cohort import enqueue_cohorts_to_calculate, reset_stuck_cohorts
...
def poll_query_performance(last_known_run_time_ns: int) -> None:
    ...
    try:
        ...
        from src.skills.analysis.posthog.tasks.poll_query_performance import poll_query_performance as poll_query_performance_nontask
...
def process_scheduled_changes() -> None:
    from src.skills.analysis.posthog.tasks.process_scheduled_changes import process_scheduled_changes
...
def sync_insight_cache_states_task() -> None:
    from src.skills.analysis.posthog.caching.insight_caching_state import sync_insight_cache_states
...
def update_cache_task(caching_state_id: UUID) -> None:
    from src.skills.analysis.posthog.caching.insight_cache import update_cache
...
def sync_insight_caching_state(
    ...
    from src.skills.analysis.posthog.caching.insight_caching_state import sync_insight_caching_state
...
def calculate_decide_usage() -> None:
    from src.skills.analysis.posthog.models.feature_flag.flag_analytics import (
...
def find_flags_with_enriched_analytics() -> None:
    ...
    from src.skills.analysis.posthog.models.feature_flag.flag_analytics import find_flags_with_enriched_analytics
...
def demo_reset_master_team() -> None:
    from src.skills.analysis.posthog.tasks.demo_reset_master_team import demo_reset_master_team
...
def sync_all_organization_available_product_features() -> None:
    from src.skills.analysis.posthog.tasks.sync_all_organization_available_product_features import (
...
def check_async_migration_health() -> None:
    from src.skills.analysis.posthog.tasks.async_migrations import check_async_migration_health
...
def verify_persons_data_in_sync() -> None:
    from src.skills.analysis.posthog.tasks.verify_persons_data_in_sync import verify_persons_data_in_sync as verify
...
def stop_surveys_reached_target() -> None:
    from src.skills.analysis.posthog.tasks.stop_surveys_reached_target import stop_surveys_reached_target
...
def update_survey_iteration() -> None:
    from src.skills.analysis.posthog.tasks.update_survey_iteration import update_survey_iteration
...
def update_survey_adaptive_sampling() -> None:
    from src.skills.analysis.posthog.tasks.update_survey_adaptive_sampling import update_survey_adaptive_sampling

    update_survey_adaptive_sampling()


def recompute_materialized_columns_enabled() -> bool:
    from src.skills.analysis.posthog.models.instance_setting import get_instance_setting
...
def send_org_usage_reports() -> None:
    from src.skills.analysis.posthog.tasks.usage_report import send_all_org_usage_reports
...
def _queue_delete_team_recordings(team_ids: list[int], deleted_by: str) -> None:
    ...
    from src.skills.analysis.posthog.temporal.common.client import async_connect
    from src.skills.analysis.posthog.temporal.session_replay.delete_recordings.types import DeletionConfig, RecordingsWithTeamInput
...
def _delete_teams_and_data(team_ids: list[int], user_id: int, project_id: int | None = None) -> None:
    ...
    """
    from src.skills.analysis.posthog.models.async_deletion import AsyncDeletion, DeletionType
    from src.skills.analysis.posthog.models.project import Project
    from src.skills.analysis.posthog.models.team import Team
    from src.skills.analysis.posthog.models.team.util import (
...
def delete_project_data_and_notify_task(
    ...
    """
    from src.skills.analysis.posthog.email import is_email_available
    from src.skills.analysis.posthog.tasks.email import send_project_deleted_email
...
def delete_organization_data_and_notify_task(
    ...
    """
    from src.skills.analysis.posthog.email import is_email_available
    from src.skills.analysis.posthog.event_usage import report_organization_deletion_completed
    from src.skills.analysis.posthog.models.organization import Organization
    from src.skills.analysis.posthog.tasks.email import send_organization_deleted_email
...
def sync_feature_flag_last_called(self: PushGatewayTask) -> None:
    ...
    from src.skills.analysis.posthog.clickhouse.client import sync_execute
    from src.skills.analysis.posthog.models.feature_flag.feature_flag import FeatureFlag
...
def refresh_activity_log_fields_cache(flush: bool = False, hours_back: int = 14) -> None:
    ...
    from src.skills.analysis.posthog.api.advanced_activity_logs.constants import BATCH_SIZE, SAMPLING_PERCENTAGE, SMALL_ORG_THRESHOLD
    from src.skills.analysis.posthog.api.advanced_activity_logs.field_discovery import AdvancedActivityLogFieldDiscovery
    from src.skills.analysis.posthog.api.advanced_activity_logs.fields_cache import delete_cached_fields
    from src.skills.analysis.posthog.models import Organization
    from src.skills.analysis.posthog.models.activity_logging.activity_log import ActivityLog
...
def sync_user_product_lists_for_new_team(team_id: int) -> None:
    ...
    """
    from src.skills.analysis.posthog.models.file_system.user_product_list import backfill_user_product_list_for_new_user
    from src.skills.analysis.posthog.models.team import Team
