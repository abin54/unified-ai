...
if TYPE_CHECKING:
    from src.skills.analysis.posthog.temporal.data_imports.cdc.types import CDCStreamReader
...
class PostgresCDCAdapter:
    ...
    def create_reader(self, source: ExternalDataSource) -> CDCStreamReader:
        from src.skills.analysis.posthog.temporal.data_imports.sources.postgres.cdc.stream_reader import (
    ...
    def drop_resources(self, conn: Any, slot_name: str, pub_name: str) -> None:
        from src.skills.analysis.posthog.temporal.data_imports.sources.postgres.cdc.slot_manager import drop_slot_and_publication

        drop_slot_and_publication(conn, slot_name, pub_name)

    def get_lag_bytes(self, conn: Any, slot_name: str) -> int | None:
        from src.skills.analysis.posthog.temporal.data_imports.sources.postgres.cdc.slot_manager import get_slot_lag_bytes
