...
class CDCExtractionWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: CDCExtractionInput) -> None:
        from src.skills.analysis.posthog.temporal.data_imports.cdc.activities import CDCExtractInput, cdc_extract_activity
...
class CDCSlotCleanupWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: None = None) -> None:
        from src.skills.analysis.posthog.temporal.data_imports.cdc.activities import cleanup_orphan_slots_activity
