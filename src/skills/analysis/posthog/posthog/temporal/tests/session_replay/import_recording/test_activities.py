...
from src.skills.analysis.posthog.temporal.session_replay.import_recording.activities import (
...
from src.skills.analysis.posthog.temporal.session_replay.import_recording.types import ImportContext, ImportRecordingInput
...
async def test_import_recording_data_success(tmp_path):
    ...
    with (
        patch(
            "posthog.temporal.session_replay.import_recording.activities.recording_s3_client.async_recording_s3_client"
        ) as mock_async_client,
        patch("posthog.temporal.session_replay.import_recording.activities.Path") as mock_path_cls,
    ):
...
async def test_import_replay_clickhouse_rows_success(tmp_path):
    ...
    with (
        patch("posthog.temporal.session_replay.import_recording.activities.get_client") as mock_get_client,
        patch("posthog.temporal.session_replay.import_recording.activities.Path") as mock_path_cls,
    ):
...
async def test_import_event_clickhouse_rows_no_file(tmp_path):
    ...
    with patch("posthog.temporal.session_replay.import_recording.activities.Path") as mock_path_cls:
