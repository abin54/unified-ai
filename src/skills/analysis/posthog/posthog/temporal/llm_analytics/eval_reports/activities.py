...
from src.skills.analysis.posthog.temporal.llm_analytics.eval_reports.constants import DOGFOOD_TEAM_IDS
from src.skills.analysis.posthog.temporal.llm_analytics.eval_reports.types import (
...
async def fetch_due_eval_reports_activity(
    ...
    from src.skills.analysis.posthog.temporal.llm_analytics.eval_reports.metrics import record_coordinator_reports_found
...
async def run_eval_report_agent_activity(
    ...
    async with Heartbeater():
        ...
        @database_sync_to_async(thread_sensitive=False)
        def run_agent():
            from src.skills.analysis.posthog.temporal.llm_analytics.eval_reports.report_agent import run_eval_report_agent
...
async def deliver_report_activity(
    ...
    async with Heartbeater():
        ...
        @database_sync_to_async(thread_sensitive=False)
        def deliver():
            from src.skills.analysis.posthog.temporal.llm_analytics.eval_reports.delivery import deliver_report
