...
from src.skills.analysis.posthog.async_migrations.definition import AsyncMigrationOperation
from src.skills.analysis.posthog.async_migrations.setup import DEPENDENCY_TO_ASYNC_MIGRATION
from src.skills.analysis.posthog.celery import app
from src.skills.analysis.posthog.clickhouse.client import sync_execute
from src.skills.analysis.posthog.clickhouse.client.connection import make_ch_pool
from src.skills.analysis.posthog.clickhouse.query_tagging import reset_query_tags, tag_queries
from src.skills.analysis.posthog.email import is_email_available
from src.skills.analysis.posthog.models.async_migration import AsyncMigration, AsyncMigrationError, MigrationStatus
...
from src.skills.analysis.posthog.settings import (
...
def execute_op_clickhouse(
    ...
    *,
    ...
    from src.skills.analysis.posthog.clickhouse import client
...
def _get_all_shard_connections():
    from src.skills.analysis.posthog.clickhouse.client.connection import ch_pool as default_ch_pool
...
def process_error(
    ...
    if alert:
        if async_migrations_emails_enabled():
            from src.skills.analysis.posthog.tasks.email import send_async_migration_errored_email
    ...
    from src.skills.analysis.posthog.async_migrations.runner import attempt_migration_rollback
...
def trigger_migration(migration_instance: AsyncMigration, fresh_start: bool = True):
    from src.skills.analysis.posthog.tasks.async_migrations import run_async_migration
...
def complete_migration(migration_instance: AsyncMigration, email: bool = True):
    ...
    if needs_update:
        ...
        if email and async_migrations_emails_enabled():
            from src.skills.analysis.posthog.tasks.email import send_async_migration_complete_email
    ...
    if get_instance_setting("AUTO_START_ASYNC_MIGRATIONS"):
        ...
        if next_migration:
            from src.skills.analysis.posthog.async_migrations.runner import run_next_migration
