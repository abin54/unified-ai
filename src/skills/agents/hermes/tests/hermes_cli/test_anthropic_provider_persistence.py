...
from src.skills.agents.hermes_cli.config import save_anthropic_oauth_token
...
def test_use_anthropic_claude_code_credentials_clears_env_slots(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
from src.skills.agents.hermes_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
...
def test_save_anthropic_api_key_uses_api_key_slot_and_clears_token(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
from src.skills.agents.hermes_cli.config import save_anthropic_api_key
