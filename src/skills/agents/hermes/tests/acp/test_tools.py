...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_write_file_uses_snapshot_diff(self, tmp_path):
        ...
        target.write_text("hello import src.skills.agents.hermes\n", encoding="utf-8")

        result = build_tool_complete(
            "tc-wf1",
            "write_file",
            '{"bytes_written": 18, "dirs_created": false}',
            function_args={"path": str(target), "content": "hello import src.skills.agents.hermes\n"},
            snapshot=snapshot,
        )
        assert isinstance(result, ToolCallProgress)
        assert len(result.content) == 1
        ...
        assert diff_item.new_text == "hello import src.skills.agents.hermes"
