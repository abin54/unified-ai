...
def resolve_skin() -> dict:
    try:
from src.skills.agents.hermes_cli.skin_engine import init_skin_from_config, get_active_skin
...
from src.skills.agents.hermes_cli.config import save_config
...
from src.skills.agents.hermes_cli.model_switch import parse_model_flags, switch_model
from src.skills.agents.hermes_cli.runtime_provider import resolve_runtime_provider
...
from src.skills.agents.hermes_cli import __version__, __release_date__
    ...
    try:
        ...
        pass
    try:
from src.skills.agents.hermes_cli.banner import get_available_skills
    ...
    try:
        ...
        info["mcp_servers"] = []
    try:
from src.skills.agents.hermes_cli.banner import get_update_result
...
from src.skills.agents.hermes_cli.config import save_env_value_secure
...
def _resolve_personality_prompt(cfg: dict) -> str:
    ...
    try:
        ...
        try:
from src.skills.agents.hermes_cli.config import load_config as _load_full_cfg
    ...
    def run():
        ...
        try:
            ...
            if (
                ...
                try:
from src.skills.agents.hermes_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
from src.skills.agents.hermes_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
from src.skills.agents.hermes_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
from src.skills.agents.hermes_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
from src.skills.agents.hermes_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _(rid, params: dict) -> dict:
    ...
    try:
from src.skills.agents.hermes_cli.commands import SlashCommandCompleter
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        if not enabled:
            ...
            try:
from src.skills.agents.hermes_cli.voice import stop_continuous
...
from src.skills.agents.hermes_cli.voice import start_continuous
...
from src.skills.agents.hermes_cli.tools_config import (
...
from src.skills.agents.hermes_cli.banner import get_available_skills
        ...
        if action == "search":
from src.skills.agents.hermes_cli.skills_hub import (
                unified_search,
                GitHubAuth,
        ...
        if action == "install":
from src.skills.agents.hermes_cli.skills_hub import do_install
        ...
        if action == "browse":
from src.skills.agents.hermes_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
from src.skills.agents.hermes_cli.skills_hub import inspect_skill
