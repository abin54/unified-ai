...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
from src.skills.agents.hermes_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
from src.skills.agents.hermes_cli.auth import clear_provider_auth
...
from src.skills.agents.hermes_cli import auth as hauth
    if provider_id == "nous":
from src.skills.agents.hermes_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
from src.skills.agents.hermes_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
from src.skills.agents.hermes_cli.auth import persist_nous_credentials
...
from src.skills.agents.hermes_cli.logs import _read_tail, LOG_FILES
    ...
    try:
from src.skills.agents.hermes_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
from src.skills.agents.hermes_cli.skills_config import get_disabled_skills
    config = load_config()
...
from src.skills.agents.hermes_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
