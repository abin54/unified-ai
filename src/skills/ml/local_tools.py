...
async def _write_handler(args: dict[str, Any], **_kw) -> tuple[str, bool]:
    ...
    try:
        ...
        if p.suffix == ".py":
            from src.skills.ml.edit_utils import validate_python
...
async def _edit_handler(args: dict[str, Any], **_kw) -> tuple[str, bool]:
    from src.skills.ml.edit_utils import apply_edit, validate_python
...
def get_local_tools():
    ...
    from src.orchestrator.core.tools import ToolSpec
