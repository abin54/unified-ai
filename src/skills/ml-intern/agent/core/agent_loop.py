...
from src.orchestrator.core.doom_loop import check_for_doom_loop
from src.orchestrator.core.llm_params import _resolve_llm_params
from src.orchestrator.core.prompt_caching import with_prompt_caching
from src.orchestrator.core.session import Event, OpType, Session
from src.orchestrator.core.tools import ToolRouter
from src.skills.ml.jobs_tool import CPU_FLAVORS
...
def _is_effort_config_error(error: Exception) -> bool:
    ...
    """
    from src.orchestrator.core.effort_probe import _is_invalid_effort, _is_thinking_unsupported
...
async def _heal_effort_and_rebuild_params(
    ...
    Two branches:
      ...
      • invalid-effort → re-run the full cascade probe; the result lands
        in the cache
    """
    from src.orchestrator.core.effort_probe import ProbeInconclusive, _is_thinking_unsupported, probe_effort
                ...
                if approval_required_tools:
                    ...
                    for tc, tool_name, tool_args in approval_required_tools:
                        ...
                        if tool_name == "hf_jobs" and isinstance(tool_args.get("script"), str):
                            from src.skills.ml.sandbox_tool import resolve_sandbox_script
                            sandbox = getattr(session, "sandbox", None)
